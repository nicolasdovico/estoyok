  ...
  - [x] Etapa 8: Re-organización de Navegación y Nuevas Pestañas.
    ...
    - [x] Corregidos problemas de usabilidad, visualización y cálculos en el historial de recorridos nativo en `MapaScreen.kt`:
      ...
      - [x] Mejoras en el Historial de Recorridos y Estadías (Estilo Life360):
        ...
        - [x] Backend: Resolución Dinámica de URLs de Avatares (Múltiples Entornos): Modificado el método accesor `getAvatarUrlAttribute` del modelo `User.php` para resolver la URL absoluta utilizando el esquema y host de la petición HTTP actual (`request()->schemeAndHttpHost()`) cuando haya un contexto de request activo. Esto soluciona los problemas de desajuste de red en entornos como Railway (donde `APP_URL` suele omitirse o resolverse incorrectamente como `localhost`), garantizando que los celulares físicos obtengan URLs válidas de producción.
