  ...
  - [x] Etapa 8: Re-organización de Navegación y Nuevas Pestañas.
    ...
    - [x] Corregidos problemas de usabilidad, visualización y cálculos en el historial de recorridos nativo en `MapaScreen.kt`:
      ...
      - [x] Mejoras en el Historial de Recorridos y Estadías (Estilo Life360):
        ...
        - [x] Pastillas de Infracciones y Eventos en Mapa (Android Native & Backend): Incorporadas 4 pastillas interactivas en la tarjeta de "Resumen Semanal de Conducción" (Exceso de velocidad, Distracción, Aceleración y Frenado) mostrando íconos informativos y el recuento total de eventos. Al pulsarlas, se despliega un diálogo con la explicación detallada de cada infracción. En el backend (`DriveController.php`), se simula de manera determinista un evento de distracción por manipulación de celular a mitad de trayecto. Para usuarios Premium, se reemplazaron los pines genéricos del mapa de detalles del trayecto por marcadores personalizados con vectores de Compose (`MarkerComposable` con círculos coloreados y símbolos blancos de Smartphone, Speed, TrendingUp y TrendingDown), y se rediseñaron los marcadores de inicio (círculo verde con ícono de semáforo `Traffic`) y fin del trayecto (círculo con patrón cuadriculado blanco y negro de meta de carreras). Además, se añadió el badge de conteo y chequeo de Celular en la tarjeta de resumen inferior, solucionando el falso reporte de conducción impecable en viajes con infracciones de distracción.
